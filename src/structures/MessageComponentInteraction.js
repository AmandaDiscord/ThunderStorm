...
class MessageComponentInteraction extends Interaction_1.default {
    constructor(client, data) {
        ...
        this.message = data.message ? new Message(this.client, data.message, this.channel && this.channel.id === data.message.channel_id ? this.channel : new PartialChannel(this.client, { id: data.message.channel_id, guild_id: data.message.guild_id, type: data.message.guild_id ? Constants_1.ChannelTypes[0] : Constants_1.ChannelTypes[1] })) : null;
...
MessageComponentInteraction.default = MessageComponentInteraction;
