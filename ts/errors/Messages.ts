...
const Messages = {
	CLIENT_INVALID_OPTION: (prop: string, must: any) => `The ${prop} option must be ${must}`,
	...
	INVALID_TYPE: (name: string, expected: any, an = false) => `Supplied ${name} is not a${an ? "n" : ""} ${expected}.`,
	INVALID_ELEMENT: (type: any, name: string, elem: any) => `Supplied ${type} ${name} includes an invalid element: ${elem}`,
